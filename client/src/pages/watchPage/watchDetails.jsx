...
function Details(props){
    ...
    useEffect(()=>{
        try{
            ...
            axios.get(`https://youtube.googleapis.com/youtube/v3/videos?part=snippet%2CcontentDetails%2Cstatistics&id=${props.broadcast.path}&key=${YOUTUBE_API}`)
                .then((res)=>{ setDesc(res.data.items[0].snippet.description)})
        } catch(error){
    ...
    return (
        <div className="w-full h-fit sm:w-11/12 border sm:border-t-2 border-t-0 border-gray-400 dark:border-white bg-amber-400 
            ...
            <div className="w-full min-h-fit h-5/6 border-t sm:border-t-0 sm:border-l sm:pl-10 border-gray-700 
                ...
                <p className="text-gray-800 dark:text-gray-300 overflow-hidden shadow-md bg-gray-200 dark:bg-gray-800 p-2 sm:p-4 rounded-md">{desc}</p>
            </div>
        </div>
    )
}
